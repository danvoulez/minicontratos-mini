...
export async function POST(request: Request) {
  ...
  try {
    ...
    const found = await db.select().from(ledgerObject).where(eq(ledgerObject.id, objectId));
    if (found.length === 0) return NextResponse.json({ error: "not_found", message: "object not found" }, { status: 404 });
    ...
    const [updated] = await db
      .update(ledgerObject)
      .set({ data: newData, version: (found[0].version ?? 1) + 1, updatedAt: new Date() })
      .where(eq(ledgerObject.id, objectId))
      .returning();

    // POST success log
    const __logTypeId = await ensureLogTypeId(db);
    await db.insert(ledgerObject).values({ typeId: __logTypeId, data: { level: 'info', message: 'ledger.transactions.post', context: { objectId, operationType, version: updated?.version } } });
    return NextResponse.json(updated, { status: 200 });
  } catch (e:any) {
    const __logTypeId = await ensureLogTypeId(db);
    await db.insert(ledgerObject).values({ typeId: __logTypeId, data: { level: 'error', message: 'ledger.transactions.post.error', context: { objectId, operationType, error: String(e?.message||e) } } });
    return NextResponse.json({ error: "record_failed", message: e?.message || "Unknown error" }, { status: 500 });
  } finally {
