...
import { hasPermission, canPromote } from "../../lib/memory/rbac";
import { encrypt, decrypt, shouldEncrypt } from "../../lib/memory/encryption";
import { detectSchemaId, validateMemoryContent } from "../../lib/memory/schema-registry";
...
test("schema detection from key patterns", () => {
  expect(detectSchemaId("user:123:preference:theme")).toBe("UserPreferenceSchema");
  expect(detectSchemaId("project:456:architecture:backend")).toBe("ArchitectureSchema");
  expect(detectSchemaId("org:789:policy:code_review")).toBe("PolicySchema");
  expect(detectSchemaId("session:abc:context:current_file")).toBe("ContextSchema");
